...
test("shows no products message when search has no match", async () => {
  const user = userEvent.setup();
  renderApp();
  ...
  await screen.findByText(/Fresh Swiss Milk/i);

  const searchBox = screen.getByPlaceholderText(/search/i);
  await user.type(searchBox, "pizza");

  expect(screen.getByText(/No dairy products found/i)).toBeInTheDocument();
});
